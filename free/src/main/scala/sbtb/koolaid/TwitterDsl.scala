...
object TwitterDsl {
  ...
  def tell(text: String, default: String = ""): Free[Instruction, Unit] = ???
  def ask(default: String): Free[Instruction, String] = ???

  def getTweets(screenNames: Seq[String]): Free[Instruction, Future[Seq[Tweets]]] = ???
  def displayTweets(tweets: Future[Seq[Tweets]]): Free[Instruction, Future[Unit]] = ???
